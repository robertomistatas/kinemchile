...
export function PacienteCombobox({
  ...
  const filteredPacientes = React.useMemo(() => {
    // Si no hay pacientes, devolver un array vacío
    if (!pacientes || pacientes.length === 0) {
      return []
    }

    // Si no hay término de búsqueda, devolver todos los pacientes
    ...
    return pacientes.filter((paciente) => {
      ...
      if (!paciente || !paciente.nombre || !paciente.apellido || !paciente.rut) {
        console.log("Paciente con datos incompletos:", paciente)
        return false
      }
    ...
    <div className="relative w-full">
      {debugMode && (
        <div className="absolute -top-6 left-0 right-0 bg-yellow-100 text-xs p-1 rounded">
          Modo debug: {pacientes?.length || 0} pacientes | Filtrados: {filteredPacientes?.length || 0}
        </div>
      )}
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            disabled={disabled}
            onClick={() => {
              // Al hacer clic, verificar si hay pacientes disponibles
              if (pacientes?.length === 0) {
                console.log("No hay pacientes disponibles para buscar")
              }
            }}
          >
            {selectedPaciente
          ...
          <Command>
            <div className="flex items-center border-b px-3">
              <Search
                ...
                onClick={() => {
                  if (!debugMode) {
                    console.log("Haz doble clic para activar el modo debug")
                  }
                }}
              />
              <CommandInput
                ...
                <div className="py-3 px-4 text-center">
                  ...
                  {debugMode && (
                    <div className="text-xs text-left bg-gray-100 p-2 mb-2 rounded">
                      ...
                      <p>Total pacientes: {pacientes?.length || 0}</p>
